...
var historicalBooks = {
    ...
    'Tobit': 'God sends the angel Raphael, disguised as a human, to heal Tobit and free Sarah from the demon. The main narrative is dedicated to Tobit\'s son, Tobiah or Tobiyah (Greek: Tobias), who is sent by his father to collect money that the elder has deposited in distant Media.',
    'Judith': 'The Book of Judith is a deuterocanonical book, included in the Roman Catholic and Eastern Orthodox Bibles, but excluded by Jews and Protestants. However, it remains a popular and widely read work among the apocrypha of the Old Testament. The story revolves around Judith, a daring and beautiful widow.',
    ...
    '1 Maccabees': '1 Maccabees is a book written in Hebrew by a Jewish author after the restoration of an independent Jewish kingdom by the Hasmonean dynasty, about the latter part of the 2nd century BC. The original Hebrew is lost and the most important surviving version is the Greek translation contained in the Septuagint.',
    '2 Maccabees': '2 Maccabees is a deuterocanonical book which focuses on the Maccabean Revolt against Antiochus IV Epiphanes and concludes with the defeat of the Seleucid empire general Nicanor in 161 BC by Judas Maccabeus, the hero of the hard work.'
};
var wisdomBooks = {
    ...
    'Wisdom of Solomon': 'It is more an exhortation to pursue wisdom than a collection of wise teachings (as in Proverbs, Sirach, and Ecclesiastes). Its implied author is King Solomon, and its implied audience is the rulers of the earth.',
    'Sirach': 'The Book of the All-Virtuous Wisdom of Yeshua ben Sira, commonly called the Wisdom of Sirach or simply Sirach, and also known as the Book of Ecclesiasticus (abbreviated Ecclus.) or Ben Sira, is a work of ethical teachings, from approximately 200 to 175 BCE, written by the Jewish scribe Ben Sira of Jerusalem, on the inspiration of his father Joshua son of Sirach, sometimes called Jesus son of Sirach or Yeshua ben Eliezer ben Sira.'
};
var propheticBooks = {
    'Isiah': 'Isaiah son of Amoz is often thought of as the greatest of the writing prophets. His name means "The Lord saves." Isaiah is a book that unveils the full dimensions of God\'s judgment and salvation.',
    ...
    'Baruch': 'The Book of Baruch is a Deuterocanonical book of the Bible, which is held as scripture by both Roman Catholic and Eastern Orthodox churches. (Protestants hold it as one of the Apocrypha.) It claims to be written by the prophet Jeremiah\'s scribe, Baruch, in 582 BC.',
    ...
    'Nahum': 'The book Nahum contains the "vision of Nahum," whose name means "comfort." The focal point of the entire book is the Lord\'s judgment on Nineveh for her oppression, cruelty, idolatry, and wickedness.',
...
var toCompare;
var badGuesses;
var bookNames = Object.keys(bible); // Get all 73 bible books in an array
var remaining = bookNames.length;
var wins, answered;

var displayVictory = () => {
    var play = confirm("Congratulations! You answered " + wins + " of 73 books.")
    if (play) {
        initialize();
    } else {
        return;
...
function updateScreen() {
    ...
    if (remaining > 0) { // choose a book at random
        
        var i = Math.floor( Math.random() * remaining );
        var book = bookNames[i];
        console.log("book ", bible[book])
        // Display notecard prompt for selected book
        // var test = document.getElementsByClassName('description')[0].innerHTML;
        // console.log("testing 123 ", test);
        document.getElementById("description").innerHTML = bible[book];
        displayHiddenWord(book);
    } else {
        displayVictory();
    }
    return;
...
var displayHiddenWord = (book) => {
    return;
}
//     console.log("Secret word looks like this: ", secretWord.join(" "));

//     // fill in underscores for blank letters of the hidden book of the Bible
//     for(var i=0; i < book.length; i++) {
//         (book[i] === " ") ? secretWord.push("<span class='tab char'>&#09;</span>") : secretWord.push("<span class='char'>___&nbsp;&nbsp;</span>");
//     }

//     // show secret word on page
//     //document.getElementsByClassName("word")[0].innerHTML = secretWord.join("");
//     processGuess(book);
//     return;
// }

// var processGuess = (book) => {

//     // make an array to capture screen internal to the program
//     for (var i = 0; i < book.length; i++) { toCompare.push(book[i]); }
//     console.log("You are in processGuess function now ", toCompare.join(""));

//     // wait for user to hit a key
//     document.addEventListener("keyup", function (event) {

//         //remaining -= 1;  // if they get it right

//         // Derive & capitalize letter from key code
//         var keychar = String.fromCharCode(event.keyCode).toUpperCase(); 
//        console.log("raw key ", event.key, " processed key char ", keychar);

//        return;
//         // If letter has not already been guessed and is contained in the book name
//         if ( bk.includes(keychar) && !lttrsAlreadyGuessed.includes(keychar) ) {
//             // loop through the book, search and replace any underscores of hidden book name with key character
//             //var count = 0;
//             while (bk.indexOf(keychar)) { // && count < book.length
//                 var i = bk.indexOf(keychar);
//                 console.log("i is ", i);
//                 // remove the key code character from the book name so that the while loop can continue
//                 if (i >= 0)
//                 bk = `${bk.slice(0, i)}${bk.slice(++i, bk.length)}`.trim();
//                 console.log("bk value is ", bk);
//                 // replace the guessStr with the letter
//                 //guessStr = guessStr.slice(0, i) + keychar + guessStr.slice((i + 1), guessStr.length);
//                 guessStr = `${guessStr.slice(0,i)}${keychar}${guessStr.slice(i,guessStr.length)}`;
//                 //count += 1;
//             }
//             document.getElementsByClassName("word")[0].innerHTML = guessStr;
//             lttrsAlreadyGuessed.concat(' ', keychar);
//             numGuessesRemaining--;
//         } else {
//             badGuesses.push(keychar);
//         }
//         alert(guessStr + " book: " + book + " bk: " + bk);

//     });
//     console.log("this is it ", guessStr); //    _ 	_ _ _ _ _ _ _ _ _ _ _ _ _ for 1 Thessalonians
//     document.getElementsByClassName("word")[0].innerHTML = guessStr;
//     //return;
// }
...
var displayVictory = () => {
    //
    alert("Congratulations, you have successfully reviewed all the books of the Bible!")
    return;
}

var initialize = function () { // Immediately-invoked function expression
    ...
    var play = confirm("To play Bible trivia, read the description and press any key to guess the book in the Bible.");
    if (play) {
        updateScreen();
    }
}();

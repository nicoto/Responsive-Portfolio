...
const bible = {'1 Peter': 'This book of the Bible is a General Epistle (Apostolic Letter). It was written to all believers in general. The author is Peter who wrote it about 60 A.D. The key personalities are the Apostle Peter, Silas, and Mark. Its purpose was to encourage suffering Christians and to call them to personal holiness; Peter’s central focus is persecution.',
'1 Thessalonians': 'The book of 1st Thessalonians is a Pauline Epistle (letter from Paul). The Apostle Paul wrote it about 52-54 A.D. and it was one of his earliest written letters. The key personalities in this book are the Apostle Paul, Timothy, and Silas. Paul wrote this letter to strengthen and encourage the church in Thessalonica. To encourage and hearten the believers, Paul chose to emphasize the second coming of Jesus Christ. Throughout this letter, Paul focused on the principles of Faith, Hope, and Love.'}
var bookNamesArray; // 73 at start

var correct = 0;
var answered = 0;

var lttrsAlreadyGuessed = "";
var numGuessesRemaining = 0;
var badGuesses = [];

var setUp = function () { // Immediately-invoked function expression
    bookNamesArray = Object.keys(bible); // Get all 73 bible books in an array
    alert("Click to begin the game");
...
var updateScreen = () => {
    // pick a random word.
    console.log("Enter a letter for a guess to match the following description.");

    // choose a book at random
    if (bookNamesArray.length > 0) {
        var i = Math.floor(Math.random() * bookNamesArray.length);
        var book = bookNamesArray[i];
        displayNewGuess(book);
    } else {
        displayVictory();
    }
    return;
...
var displayNewGuess = (book) => {

    document.getElementsByClassName("description")[0].innerHTML = bible[book];

    // fill in underscores for hidden book of the Bible
    var guessStr = "";

    numGuessesRemaining = book.length;
    for (var i = 0; i < book.length; i++) {
        (book[i] === " ") ? guessStr += "\t" : guessStr += "_ ";
    }
    console.log("guess string ", guessStr);
    //document.getElementsByClassName("word")[0].innerHTML = guessStr;
    processGuess(book, guessStr);
    return;
}

var processGuess = (book, guessStr) => {

    // book name copy to record eliminating characters typed by user
    var bk = book.slice(0, book.length).toUpperCase();
    // set total guesses of book
    numGuessesRemaining = book.length;
